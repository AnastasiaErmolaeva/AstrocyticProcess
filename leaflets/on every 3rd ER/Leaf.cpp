...
void Leaf::init(const int& ind_in, Cell* astrocyte_in, const vector<Cell*>& leafArr_in)
{
	...
	if (isInputImpulse)
		impulse.init(impulseAmplitude, impulseDuration, impulseStartTime, impulseFreq, impulsesQuantity, FrequencyType(freqTypeInt), AmplitudeType(ampTypeInt), impMinAmp, impMaxAmp, timeStep, my_ind + seed);
	else
		impulse.init(impulseAmplitude, impulseDuration, impulseStartTime, impulseFreq, impulsesQuantity, FREQ_NEVER, AmplitudeType(ampTypeInt), impMinAmp, impMaxAmp, timeStep, my_ind + seed);
}
...
void Leaf::initConnectedAstroParts()
{
	...
	int iAstroPart = -1, nConnLeafs, leafInd;
	connAstroParts.nConnectedAstroParts = 0;
	while (leaf2astroPartConnections >> nConnLeafs)
	{
		++iAstroPart;
		for (int i = 0; i < nConnLeafs; ++i)
		{
			leaf2astroPartConnections >> leafInd;
			if (leafInd == my_ind)
			{
