...
public:		//functions
	...
	const FP& getNa(const int& iPart) { return my_inf; }
	const FP& getH(const int& iPart) { return my_inf; }
	const FP& getN2(const int& iPart) { return my_inf; }

	const FP& getCa_old(const int& iPart) { return my_inf; }
	const FP& getNa_old(const int& iPart) { return my_inf; }
	const FP& getH_old(const int& iPart) { return my_inf; }
...
private:	//functions
	...
	inline FP Ca_tau(const FP& qold)
	{
		return (Ca_rest - qold) / tau_Ca;
	}

	inline FP Noise(const FP& nold, const FP& qold)
	{
