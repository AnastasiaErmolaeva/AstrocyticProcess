...
void Astrocyte::init(const vector<Cell*>& tailNet_in)
{
	...
	for (int iPart = 0; iPart < nAstroParts; ++iPart)
	{
		...
		astroParts[iPart].impulseComp2.init(impulseAmplitudeComp, impulseDurationComp, impulseStartTimeComp, impulseFreqComp, impulsesQuantityComp, FrequencyType(freqTypeIntComp), AmplitudeType(ampTypeIntComp), impMinAmpComp, impMaxAmpComp, timeStep, seed + nAstroParts + iPart);
	}
...
{
	if ((iPart == 0) || (iPart == 2) || (iPart == 3) || (iPart == 6) || (iPart == 7) || (iPart == 8) || (iPart == 9) || (iPart == 11) || (iPart == 12) || (iPart == 13) || (iPart == 14) || (iPart == 15) || (iPart == 18) || (iPart == 19) || (iPart == 21) || (iPart == 23) || (iPart == 26) || (iPart == 27) || (iPart == 28) || (iPart == 30) || (iPart == 31) || (iPart == 33) || (iPart == 34) || (iPart == 36) || (iPart == 38) || (iPart == 39) || (iPart == 41) || (iPart == 43) || (iPart == 44) || (iPart == 45) || (iPart == 47) || (iPart == 49))
	{
		param1[iPart] = 1.;
	}
	else
	...
	if ((iPart == 0) || (iPart == 9) || (iPart == 23) || (iPart == 38))
	{
		param2[iPart] = 1.;
	}
	else
	...
	if (iStep == 0)
	{
		...
		Imp_comp2 = astroParts[iPart].impulseComp2.createImpulse(timeAstro, true);
	}
	else if ((iStep == 1) || (iStep == 2))
	{
		Imp_comp = astroParts[iPart].impulseComp.createImpulse(timeAstro + timeStep / 2., false);
		Imp_comp2 = astroParts[iPart].impulseComp2.createImpulse(timeAstro + timeStep / 2., false);
	}
	else
	{
		Imp_comp = astroParts[iPart].impulseComp.createImpulse(timeAstro + timeStep, false);
		Imp_comp2 = astroParts[iPart].impulseComp2.createImpulse(timeAstro + timeStep, false);
	}	
	
	calcPin(iPart, iStep);
	calcQin(iPart, iStep);

	astroParts[iPart].k[0] = (astroParts[iPart].p_in + (p_1 - (astroParts[iPart].p_old + astroParts[iPart].rungeKuttCoeffMult[iStep] * astroParts[iPart].k_old[0])) * Tr + Jplc(astroParts[iPart].q_old + astroParts[iPart].rungeKuttCoeffMult[iStep] * astroParts[iPart].k_old[1]) + param1[iPart] * prod_PLC_b(Imp_comp) + param2[iPart] * prod_PLC_b(Imp_comp2)) * timeStep;
